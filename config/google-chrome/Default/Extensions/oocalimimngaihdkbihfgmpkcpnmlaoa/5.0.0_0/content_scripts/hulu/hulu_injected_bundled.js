    ...
    switch (event_type) {
        case "SEEK": {
            if (IS_SAFARI) {
                ...
                document.querySelector("#content-video-player").__HuluDashPlayer__.currentTime =
                    event.data.time / 1000 + window.offset;
            }
            break;
        }
        case "UpdateState": {
            if (IS_SAFARI) {
                ...
                const offset = video.__HuluDashPlayer__._player.currentTimeInTimeline -
                    video.__HuluDashPlayer__._player.currentTimeInStream;
                if (offset < 50) {
